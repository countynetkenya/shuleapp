...
*/
		...
		if(permissionChecker('terminalreport')) {
			if($_POST) {
			    ...
			    exit;
					...
					$subjecttype                			= pluck($this->subject_m->get_order_by_subject_with_subjectteacher(array('classesID' => $classesID, 'schoolID' => $schoolID)),'type','subjectID');
	...
	public function pdf() {
		if(permissionChecker('terminalreport')) {
			...
			$rankingType  = htmlentities(escapeString($this->uri->segment(8)));
			$schoolyearID = $this->session->userdata('defaultschoolyearID');
			$schoolID     = $this->session->userdata('schoolID');
			...
			if((int)$examID && (int)$classesID && ((int)$sectionID || $sectionID >= 0) && ((int)$studentID || $studentID >= 0)) {
				...
				$this->data['sectionID']  = $sectionID;
				$this->data['studentIDD'] = $studentID;
				$this->data['rankingType'] = $rankingType;
				...
				if(customCompute($exams)) {
					foreach($exams as $exam)
						$examIDArray[] 								= $exam->examID;
					$marks                    			= $this->mark_m->get_order_by_all_student_mark_with_markrelation2(['schoolID' => $schoolID, 'schoolyearID' => $schoolyearID, 'classesID' => $classesID, 'examID' => $examIDArray]);
				}
				if($examcompilationID > 0)
					$marks                  			  = $this->getMarkWithExam($marks, $examcompilationID);
				else
				...
				$this->subject_m->order('type DESC');
				$this->data['subjects'] 					= $this->subject_m->get_order_by_subject_with_subjectteacher(array('classesID' => $classesID, 'schoolID' => $schoolID));
					...
					foreach($exams as $exam) {
						if(customCompute($students)) {
							foreach($students as $student) {
									...
									foreach ($mandatorySubjects as $mandatorySubject) {
										...
										if(customCompute($markpercentages)) {
											foreach ($markpercentages as $markpercentageID) {
												...
												if($rankingType == "all" || $rankingType == "mandatory" || $rankingType == "mixed") {
													if(isset($studentPosition[$exam->examID][$student->srstudentID]['rankingMandatorySubjectMark'][$mandatorySubject->subjectID])) {
														if(isset($marks[$exam->examID][$student->srstudentID][$mandatorySubject->subjectID][$markpercentageID]) && $f) {
															$studentPosition[$exam->examID][$student->srstudentID]['rankingMandatorySubjectMark'][$mandatorySubject->subjectID] += $marks[$exam->examID][$student->srstudentID][$mandatorySubject->subjectID][$markpercentageID];
														} else {
															$studentPosition[$exam->examID][$student->srstudentID]['rankingMandatorySubjectMark'][$mandatorySubject->subjectID] += 0;
														}
													} else {
														if(isset($marks[$exam->examID][$student->srstudentID][$mandatorySubject->subjectID][$markpercentageID]) && $f) {
															$studentPosition[$exam->examID][$student->srstudentID]['rankingMandatorySubjectMark'][$mandatorySubject->subjectID] = $marks[$exam->examID][$student->srstudentID][$mandatorySubject->subjectID][$markpercentageID];
												...
												if(!isset($studentChecker['subject'][$exam->examID][$student->srstudentID][$markpercentageID]) && $f) {
													if($student->sroptionalsubjectID != 0) {
														...
														if($rankingType == "all" || $rankingType == "optional") {
															if(isset($studentPosition[$exam->examID][$student->srstudentID]['rankingOptionalSubjectMark'][$student->sroptionalsubjectID])) {
																if(isset($marks[$exam->examID][$student->srstudentID][$student->sroptionalsubjectID][$markpercentageID])) {
																	$studentPosition[$exam->examID][$student->srstudentID]['rankingOptionalSubjectMark'][$student->sroptionalsubjectID] += $marks[$exam->examID][$student->srstudentID][$student->sroptionalsubjectID][$markpercentageID];
																} else {
																	$studentPosition[$exam->examID][$student->srstudentID]['rankingOptionalSubjectMark'][$student->sroptionalsubjectID] += 0;
																}
															} else {
																if(isset($marks[$exam->examID][$student->srstudentID][$student->sroptionalsubjectID][$markpercentageID])) {
																	$studentPosition[$exam->examID][$student->srstudentID]['rankingOptionalSubjectMark'][$student->sroptionalsubjectID] = $marks[$exam->examID][$student->srstudentID][$student->sroptionalsubjectID][$markpercentageID];
								...
								if($rankingType == "all" || $rankingType == "mandatory" || $rankingType == "mixed") {
									$rankingMandatorySubjectMarks = $studentPosition[$exam->examID][$student->srstudentID]['rankingMandatorySubjectMark'];
									foreach($rankingMandatorySubjectMarks as $key => $value) {
										$studentPosition[$exam->examID][$student->srstudentID]['totalRankingSubjectMark'] += $value;
										$studentSubjectRanked[$exam->examID][$student->srstudentID][$key] = "Yes";
									}
									if($rankingType == "all") {
										$rankingOptionalSubjectMarks = $studentPosition[$exam->examID][$student->srstudentID]['rankingOptionalSubjectMark'];
										foreach($rankingOptionalSubjectMarks as $key => $value) {
											$studentPosition[$exam->examID][$student->srstudentID]['totalRankingSubjectMark'] += $value;
											$studentSubjectRanked[$exam->examID][$student->srstudentID][$key] = "Yes";
										}
									} elseif($rankingType == "mixed") {
										$rankingOptionalSubjectMarks = $studentPosition[$exam->examID][$student->srstudentID]['rankingOptionalSubjectMark'];
										if(customCompute($rankingOptionalSubjectMarks)) {
											asort($rankingOptionalSubjectMarks);
											$rankingOptionalSubjectMarks = array_slice($$rankingOptionalSubjectMarks, -4);
											foreach($rankingOptionalSubjectMarks as $key => $value) {
												$studentPosition[$exam->examID][$student->srstudentID]['totalRankingSubjectMark'] += $value;
												$studentSubjectRanked[$exam->examID][$student->srstudentID][$key] = "Yes";
											}
								...
								} elseif($rankingType == "optional") {
									$rankingOptionalSubjectMarks = $studentPosition[$exam->examID][$student->srstudentID]['rankingOptionalSubjectMark'];
									foreach($rankingOptionalSubjectMarks as $key => $value) {
										$studentPosition[$exam->examID][$student->srstudentID]['totalRankingSubjectMark'] += $value;
										$studentSubjectRanked[$exam->examID][$student->srstudentID][$key] = "Yes";
									}
								} elseif($rankingType == "best_optional") {
									$rankingOptionalSubjectMarks = $studentPosition[$exam->examID][$student->srstudentID]['rankingOptionalSubjectMark'];
									if(customCompute($rankingOptionalSubjectMarks)) {
										asort($rankingOptionalSubjectMarks);
										$rankingOptionalSubjectMarks = array_slice($$rankingOptionalSubjectMarks, -4);
								...
								try {
									//$studentPosition[$exam->examID][$student->srstudentID]['classPositionMark'] = ($studentPosition[$exam->examID][$student->srstudentID]['totalSubjectMark'] / customCompute($studentPosition[$exam->examID][$student->srstudentID]['subjectMark']));
									$studentPosition[$exam->examID][$student->srstudentID]['classPositionMark'] = ($studentPosition[$exam->examID][$student->srstudentID]['totalRankingSubjectMark'] / (customCompute($studentPosition[$exam->examID][$student->srstudentID]['rankingMandatorySubjectMark']) + customCompute($studentPosition[$exam->examID][$student->srstudentID]['rankingOptionallSubjectMark'])));
								} catch (DivisionByZeroError $e) {
									$studentPosition[$exam->examID][$student->srstudentID]['classPositionMark'] = 0;
								}
