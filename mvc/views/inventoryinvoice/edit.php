                    ...
                    <table class="table table-bordered product-style" style="font-size: 16px;">
                        <thead>
                            <tr>
                              ...
                              <th class="col-sm-4"><?=$this->lang->line('inventoryinvoice_product')?></th>
                              <th class="col-sm-2" ><?=$this->lang->line('inventoryinvoice_unit_price')?></th>
                              ...
                              <th class="col-sm-2"><?=$this->lang->line('inventoryinvoice_subtotal')?></th>
                                ...
                                if(customCompute($productsaleitems)) {
                                    ...
                                    foreach ($productsaleitems as $productsaleitem) {
                                        ...
                                        echo '<tr id="tr_'.$randID.'" saleproductid="'.$productsaleitem->productID.'">';
                                            ...
                                            echo '<td>';
                                                echo $totalProductStockQuantity[$productsaleitem->productID] + $totalEditQuantity[$productsaleitem->productID];
                                            echo '</td>';
                        ...
                        <tfoot id="productListFooter">
                            <tr>
                                <td colspan="3" style="font-weight: bold"><?=$this->lang->line('inventoryinvoice_total')?></td>
...
<script type="text/javascript">
    ...
    function productItemDesign(productID, productText) {
        ...
        var text = '<tr id="tr_'+randID+'" saleproductid="'+productID+'">';
            ...
            text += '<td>';
                text += ('<input type="text" class="form-control change-productquantity" id="productquantity_'+randID+'" max="'+ (available) +'"  data-productquantity-id="'+randID+'">' );
            text += '</td>';
    ...
    $(document).on('click', '#updatePurchaseButton', function() {
        ...
        var productitem = $('tr[id^=tr_]').map(function(){
            return { productID : $(this).attr('saleproductid'), unitprice: $(this).children().eq(2).children().val(), quantity : $(this).children().eq(4).children().val() };
        }).get();
